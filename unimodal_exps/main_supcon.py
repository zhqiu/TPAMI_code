...
from losses import BarlowTwins, VICReg, Projector, simco_loss, tau_simclr, hcl_loss, iSogCLR_Plus_Loss, Group_iSogCLR_Loss
...
def parse_option():
    ...
    # for DRO mod
    parser.add_argument('--DRO_mod', action='store_true')       # iSogCLR
    parser.add_argument('--add_surrogate_func', action='store_true')
    parser.add_argument('--new_imp_type', default='', type=str) # 'v1': iSogCLR+, 'v2': Group iSogCLR
    ...
    # for iSogCLR+
    parser.add_argument('--gamma_v', default=0.5, type=float)
    ...
    if opt.method == 'MoCo':
        ...
        if opt.BI_mod:
            ...
            if len(opt.new_imp_type) > 0:
                if opt.new_imp_type == 'v1':  # iSogCLR+
                    criterion = iSogCLR_Plus_Loss(gamma_s=opt.DRO_gamma, gamma_u=opt.DRO_gamma, beta_u=opt.beta_msvr, beta_s=opt.beta_msvr, tau_init=opt.DRO_tau_init,
                                                    eta_init=opt.DRO_eta_init, bsz=opt.batch_size)

                elif opt.new_imp_type == 'v2': # Group iSogCLR
                    with open("taus.pkl", "rb") as f:
                        taus = pickle.load(f)
                    with open("group_info_dict.pkl", "rb") as f:
                        group_info_dict = pickle.load(f)
                    criterion = Group_iSogCLR_Loss(taus, group_info_dict, eta_p=0.01, lambada=0.3, num_groups=10, rho=opt.DRO_rho, N=15000000, gamma=0.8, bsz=256)
                else:
                    assert 0
            else:
                if not opt.add_surrogate_func: # iSogCLR
...
def train(train_loader, model, criterion, optimizer, epoch, max_epoch, opt, scaler=None, model_last=None, optimizer_last=None):
    """one epoch training"""
    model.train()
    model.temp_generator.train()

    if model_last is not None: # iSogCLR+
        model_last.train()
        ...
        if opt.use_amp:
            with torch.cuda.amp.autocast():
                ...
                if opt.method == 'MoCo':
                    ...
                    if opt.DRO_mod and opt.new_imp_type=='v1': # for iSogCLR+
                        _, raw_features_last = model_last(images)
                        norm_features_last = F.normalize(raw_features_last, dim=1)
                        f1_last, f2_last = torch.split(norm_features_last, [bsz, bsz], dim=0)
                        features_last = torch.cat([f1_last.unsqueeze(1), f2_last.unsqueeze(1)], dim=1)
                    ...
                    if opt.method == 'FlatCLR':
                        ...
                        if opt.BI_mod:
                            ...
                            if opt.new_imp_type == 'v1':          # iSogCLR+
                                if epoch == 1:
                                    _, _, loss, _, avg_tau = criterion(index, features, epoch, max_epoch)
                                    losses.update(loss.item(), bsz)
                                    scaler.scale(loss).backward()
                                    scaler.step(optimizer)
                                    scaler.update()
                                else:
                                    last_g, last_grad_tau, last_loss, _, _ = criterion(index, features_last, epoch, max_epoch, compute_last=True)
                                    _, _, loss, ma_dict, avg_tau = criterion(index, features, epoch, max_epoch, last_g, last_grad_tau)
                                ...
                                loss, p = criterion(index, features, epoch, max_epoch)
                                #print(p)
            ...
            if not (opt.DRO_mod and opt.new_imp_type=='v1'):  # for iSogCLR+, we need implement its STORM update
                # update metric
                losses.update(loss.item(), bsz)
                    ...
                    if opt.BI_mod:
                        ...
                        if opt.new_imp_type == 'v1':    # iSogCLR+
                            if epoch == 1:
                                ...
                                grad_list = torch.autograd.grad(last_loss, model_last.parameters())
...
def main():
    ...
    if opt.DRO_mod and opt.new_imp_type=='v1': # iSogCLR+
        model_last = cpoy.deepcopy(model)
        optimizer_last = set_optimizer(opt, model_last)
    else:
        model_last = None
        optimizer_last = None
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        # train for one epoch
        time1 = time.time()
        loss = train(train_loader, model, criterion, optimizer, epoch, opt.epochs, opt, scaler, model_last, optimizer_last)
        time2 = time.time()
