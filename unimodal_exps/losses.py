...
class iSogCLR_Plus_Loss(nn.Module):
    def __init__(self, gamma_s, gamma_u, beta_s, beta_u, N=15000000,
                    tau_init=0.07, tau_min=0.05, tau_max=1.0, rho=6.0, bsz=256,
                    eta_init=0.001, eta_min=1e-4, eta_sched='const', eta_exp_gamma=0.8):
        super(iSogCLR_Plus_Loss, self).__init__()
        self.gamma_s = gamma_s 
        self.gamma_u = gamma_u
        self.beta_s = beta_s      # for the MSVR estimator
        self.beta_u = beta_u      # for the MSVR estimator
        self.tau_init = tau_init
        self.tau_min = tau_min
        self.tau_max = tau_max
        self.rho = rho
    ...
    def update_ma_estimators(self, ma_update_dict):
        index = ma_update_dict['index']
        new_s, new_u = ma_update_dict['new_s'], ma_update_dict['new_u']
        self.s[index] = new_s.cuda()
        self.u[index] = new_u.cuda()

    def forward(self, index, features, epoch, max_epoch,
                last_g=None, last_grad_tau=None, compute_last=False):
        #Compute SogCLR_DRO loss
        ...
        local_g = (g.squeeze()[:bsz] + g.squeeze()[bsz:]) / 2.0

        if not compute_last:
            self.s[index] = (1-self.gamma_s) * self.s[index] + self.gamma_s * local_g

            if last_g is not None: # employing the MSVR estimator
                self.s[index] += self.beta_s * (local_g - last_g.cuda())

            s = torch.clamp(self.s[index], min=self.eps)

        else:
            s = (1-self.gamma_s) * self.s[index] + self.gamma_s * (g.squeeze()[:bsz] + g.squeeze()[bsz:]) / 2.0
            self.s[index] = s

        weights_1 = exp_sim_d_temps[:bsz, :] / s[:, None]
        loss_1 = torch.sum(weights_1 * sim[:bsz, :], dim=1, keepdim=True) / self.num_neg - pos_sim[:bsz, :]

        weights_2 = exp_sim_d_temps[bsz:, :] / s[:, None]
        loss_2 = torch.sum(weights_2 * sim[bsz:, :], dim=1, keepdim=True) / self.num_neg - pos_sim[bsz:, :]
        ...
        if not compute_last:
            self.u[index] = (1.0-self.gamma_u) * self.u[index] + self.gamma_u * grad_tau

            if last_grad_tau is not None:
                self.u[index] += self.beta_u * (grad_tau - last_grad_tau.cuda())

            self.u[index] = torch.clamp(self.u[index], min=self.eps)
        ...
        ma_update_dict = {'index': index, 'new_s': self.s[index], 'new_u': self.u[index]}
        
        return local_g, grad_tau, loss.mean(), ma_update_dict, avg_tau




class Group_iSogCLR_Loss(nn.Module):
    def __init__(self, taus, group_info_dict, eta_p, lambada, num_groups, rho, N=15000000, gamma=0.8, bsz=256):
        super(Group_iSogCLR_Loss, self).__init__()
        self.taus = taus
        self.group_info_dict = group_info_dict # dict: {index: group_id}
        ...
        self.s = torch.zeros(N).cuda()
        self.p = torch.ones(num_groups).cuda() / num_groups
        self.z = torch.zeros(num_groups).cuda()
        self.eps = 1e-8
        self.grad_clip = 3.0
    ...
    def index_to_groupid(self, index):
        group_ids = [self.group_info_dict[ind.item()] for ind in index]
    ...
    def forward(self, index, features, epoch, max_epoch):
        ...
        tau = self.taus[index].repeat(2).cuda()
        ...
        index_group_id = self.index_to_groupid(index)
        group_weight = (self.num_groups * self.p[index_group_id]).cuda()

        weights_1 = exp_sim_d_temps[:bsz, :] / s1[:, None]
        loss_1 = torch.sum(weights_1 * group_weight[:, None] * sim[:bsz, :], dim=1, keepdim=True) / self.num_neg - pos_sim[:bsz, :]

        weights_2 = exp_sim_d_temps[bsz:, :] / s2[:, None]
        loss_2 = torch.sum(weights_2 * group_weight[:, None] * sim[bsz:, :], dim=1, keepdim=True) / self.num_neg - pos_sim[bsz:, :]

        loss = loss_1 + loss_2

        # update p
        F1 = self.taus[index].cuda() * (torch.log(s1) + self.rho)
        F2 = self.taus[index].cuda() * (torch.log(s2) + self.rho)

        index_group_id_mat = F.one_hot(index_group_id)
        group_counts = torch.sum(index_group_id_mat, dim=0)

        grad_p_1 = torch.sum(index_group_id_mat * F1[None,:], dim=0) / group_counts
        grad_p_2 = torch.sum(index_group_id_mat * F2[None,:], dim=0) / group_counts

        self.z = (1-self.gamma) * self.z + self.gamma * (grad_p_1 + grad_p_2) / 2.0

        grad_hp = - self.lambada * torch.log(self.p + self.eps) - self.lambada

        new_p = self.p * torch.exp(2*self.eta_p * (self.z + grad_hp).clamp_(min=-self.grad_clip, max=self.grad_clip))
        normalizer = new_p.sum()

        self.p = new_p / normalizer
