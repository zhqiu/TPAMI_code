...
def train(model, data_loader, optimizer, tokenizer, epoch, max_epoch, warmup_steps, device, scheduler, grad_scaler, model_last):
    # train
    model.train()  
    ...
    for i,(image, text, idx, text_idx) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if grad_scaler is None:
            ...
            if args.ita_type == 'isogclr_plus': # perform STORM style update
                with torch.cuda.amp.autocast():
                    image_ids, text_ids, image_feat, text_feat = model(image, text_input, idx=idx, text_idx=text_idx, epoch=epoch, max_epoch=max_epoch)
                    _, _, image_feat_last, text_feat_last = model_last(image, text_input, idx=idx, text_idx=text_idx, epoch=epoch, max_epoch=max_epoch)

                if epoch == 0:
                    #g_I, g_T, grad_tau_image, grad_tau_text, total_loss, avg_image_tau, avg_text_tau, ma_update_dict
                    _, _, _, _, loss_ita, avg_image_tau, avg_text_tau, _ = model.criterion(image_feat, text_feat, image_ids, text_ids, epoch, max_epoch)
                    grad_scaler.scale(loss_ita).backward()
                    

                else:
                    last_g_I, last_g_T, last_grad_tau_I, last_grad_tau_T, last_loss, _, _, _ = model_last.criterion(image_feat_last, text_feat_last, image_ids, text_ids, epoch, max_epoch, compute_last=True)
                    _, _, _, _, loss_ita, avg_image_tau, avg_text_tau, ma_dict = model.criterion(image_feat, text_feat, image_ids, text_ids, epoch, max_epoch, last_g_I, last_g_T, last_grad_tau_I, last_grad_tau_T)

                    grad_last_round = torch.autograd.grad(last_loss, model_last.parameters())

                    model_last.load_state_dict(model.state_dict())
                    model_last.update_ma_estimators(ma_dict)

                    grad = torch.autograd.grad(loss_ita, model.parameters())

                    # STORM
                    for param, g, g_last_round in zip(model.parameters(), grad, grad_last_round):
                        param.grad = g - opt.gamma_v * g_last_round
...
def main(args):
    ...
    if args.ita_type == 'group_isogclr':
        with open(args.taus_path, "rb") as f:
            taus_dict = pickle.load(f)
        taus_I = taus_dict["image"]
        taus_T = taus_dict["text"]

        with open(args.group_info_path, "rb") as f:
            group_info = pickle.load(f)
        group_info_I = group_info["image"]
        group_info_T = group_info["text"]
    else:
        taus_I, taus_T = None, None
        group_info_I, group_info_T = None, None
    ...
    model = CLIP(image_encoder=args.image_encoder, text_encoder=args.text_encoder, embed_dim=args.embed_dim, init_model=args.init_model, bsz=args.batch_size_train*args.world_size,
                  ...
                  vicreg_sim_coeff=args.vicreg_sim_coeff, vicreg_std_coeff=args.vicreg_std_coeff, personalized_tau=args.personalized_tau, enable_surrogate=args.enable_surrogate, beta_msvr=args.beta_msvr,
                  taus_I=taus_I, taus_T=taus_T, group_info_I=group_info_I, group_info_T=group_info_T, eta_p=args.eta_p, lambada=args.lambada, num_groups=args.num_groups)
    ...
    for epoch in range(0, max_epoch):
        if not args.evaluate:
            ...
            train_stats = train(model, train_loader, optimizer, tokenizer, epoch, max_epoch, warmup_steps, device, lr_scheduler, grad_scaler, model_last)
...
if __name__ == '__main__':
    ...
    # loss config
    parser.add_argument('--ita_type', required=True, choices=['clip', 'cyclip', 'vicreg', 'sogclr', 'sogclr_dro', 'isogclr_plus', 'group_isogclr'])
    ...
    parser.add_argument('--beta_msvr', default=1e-3, type=float)
    parser.add_argument('--taus_path', default='', type=str)
    parser.add_argument('--group_info_path', default='', type=str)
    parser.add_argument('--eta_p', default=0.01, type=float)
    parser.add_argument('--lambada', default=0.05, type=float)
