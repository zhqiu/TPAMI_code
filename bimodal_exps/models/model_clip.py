...
from models.losses import CLIP_Loss, CyCLIP_Loss, SogCLR_Loss, SogCLR_DRO_Loss, iSogCLR_Plus_Loss, Group_iSogCLR_Loss, VICReg_Loss
...
class CLIP(nn.Module):
    def __init__(self,               
        ...
        elif self.ita_type == 'sogclr_dro': # isogclr
        ...
        elif self.ita_type == 'isogclr_plus':
            self.criterion = iSogCLR_Plus_Loss(gamma_s=sogclr_gamma, gamma_u=sogclr_gamma, beta_s=beta_msvr, beta_u=beta_msvr,
                                                world_size=world_size, rho_init=rho_init, tau_init=tau_init, bsz=bsz,
                                                eta_init=eta_init, eta_sched=eta_sched, eta_exp_gamma=eta_exp_gamma, beta_u=beta_u)
        elif self.ita_type == 'group_isogclr':
            self.criterion = Group_iSogCLR_Loss(taus_I=taus_I, taus_T=taus_T, group_info_I=group_info_I, group_info_T=group_info_T, 
                                                eta_p=eta_p, lambada=lambada, num_groups=num_groups,
                                                gamma=sogclr_gamma, rho_init=rho_init, bsz=bsz, world_size=world_size)
    ...
    def forward(self, image, text, idx, text_idx, epoch, max_epoch):
        ...
        if self.ita_type in ['clip', 'cyclip']:
            ...
            return image_ids, text_ids, image_feat, text_feat

        elif self.ita_type == 'group_isogclr':
            image_ids = concat_all_gather(idx)
            text_ids = concat_all_gather(text_idx)
            loss_ita, p_I, p_T = self.criterion(image_feat, text_feat, image_ids, text_ids, epoch, max_epoch)
